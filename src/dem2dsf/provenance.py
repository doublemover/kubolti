...
def build_provenance(
    *,
    ...
    if strict and dsftool_cmd:
        try:
            dsftool_info = tools.get("dsftool") or {"command": list(dsftool_cmd)}
            dsftool_version_value = dsftool_version(dsftool_cmd)
            if dsftool_version_value:
                dsftool_info["version"] = ".".join(str(part) for part in dsftool_version_value)
            tools["dsftool"] = dsftool_info
        except Exception:
            pass
    ...
    pinned_versions, pinned_source = load_pinned_versions(
        Path(options["pinned_versions_path"]) if options.get("pinned_versions_path") else None
    )
    warnings: list[str] = []
    if pinned_versions:

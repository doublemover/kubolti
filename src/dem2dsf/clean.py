...
def format_clean_summary(report: Mapping[str, Any]) -> list[str]:
    """Format a summary of clean results for logging."""
    build_dir = report.get("build_dir")
    dry_run = report.get("dry_run")
    include_value = report.get("include")
    include = [str(item) for item in include_value] if isinstance(include_value, list) else []
    removed_value = report.get("removed")
    removed = removed_value if isinstance(removed_value, dict) else {}
